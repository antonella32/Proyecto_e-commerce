...
// Crear elementos para mostrar el costo de envío y el costo total
const shippingCostElement = document.createElement("span");
shippingCostElement.className = "shipping-cost";
const totalCostElement = document.createElement("span");
totalCostElement.className = "total-cost";
cartItemsContainer.appendChild(shippingCostElement);
cartItemsContainer.appendChild(totalCostElement);

// Definir los costos de envío
const shippingCosts = {
  'Premium': 0.15, // 15% del subtotal
  'Express': 0.07, // 7% del subtotal
  'Standard': 0.05 // 5% del subtotal
};
...
function actualizarCostos() {
  ...
  const shippingCost = subtotal * shippingCosts[shippingType];
  const totalCost = subtotal + shippingCost;

  shippingCostElement.textContent = `Costo de envío: ${shippingCost.toFixed(2)} ${listaCarrito[0]?.moneda || ''}`;
  totalCostElement.textContent = `Total: ${totalCost.toFixed(2)} ${listaCarrito[0]?.moneda || ''}`;
}
...
function mostrarSubtotal(producto) {
  ...
  if (subtotalElement) {
    ...
    subtotalElement.innerText = `${subtotal.toFixed(2)} ${producto.moneda}`; // Muestra solo el subtotal
    actualizarCostos(); // Actualizar el costo total y el costo de envío
  }
}
...
function actualizarCantidad(nombreProducto, nuevaCantidad) {
  ...
  if (producto) {
    ...
    actualizarCostos(); // con esto se actualiza el costo total y el costo de envío
  }
}
  ...
  let item = `
     <div class="cart-item">
          ...
          <div class="incart_soldcount">
              ...
              <div data-nombre="${Originalproduct.nombre}">Subtotal: <span class="incart_subtotal" data-nombre="${Originalproduct.nombre}">${subtotal.toFixed(2)} ${Originalproduct.moneda}</span></div>
          </div>
      </div>
  `;
...
// Calcular el costo total y el costo de envío inicialmente
actualizarCostos();
...
// Agregar el botón, el costo de envío y el costo total al contenedor
buttonContainer.appendChild(botonComprar);
buttonContainer.appendChild(shippingCostElement);
buttonContainer.appendChild(totalCostElement);
...
/* ACÁ PEGO EL CÓDIGO ORIGINAL (hasta línea 105)
